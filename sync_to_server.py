...
DATA_LEDGER = BASE_DIR / "data" / "ledger.json"
...
def print_banner():
    print("=" * 65)
    print("        🚀 抖音登录凭证 + 联系人台账 (state/ledger) 云服务器一键同步工具")
    print("=" * 65)
...
def main():
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        try:
            for fname, local in uploads:
                remote_file = f"{remote_data_dir}/{fname}"
                print(f"[*] 正在上传 {fname} -> {remote_file} ...")
        ...
        print("\n" + "=" * 65)
        print("  🎉🎉 恭喜！抖音登录凭证与联系人台账已成功同步至云服务器！")
        print(f"  [✓] 远程文件位置: {remote_data_dir}/state.json")
        if DATA_LEDGER.exists():
            print(f"  [✓] 联系人台账已同步: {remote_data_dir}/ledger.json")
        print("=" * 65)
        print("\n【下一步推荐】")
