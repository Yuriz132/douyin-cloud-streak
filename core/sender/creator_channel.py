...
def _do_send(page, context, nickname: str, msg_text: str, dry_run: bool, max_scrolls: int) -> tuple[bool, str]:
    ...
    if msg_text not in cur:
        return False, "文字未能输入到 creator 聊天输入框"

    page.keyboard.press("Enter")

    # 发送后校验：输入框应清空；否则消息大概率未发出，如实报失败
    deadline = time.time() + 8
    while time.time() < deadline:
        time.sleep(1)
        try:
            if msg_text not in (input_box.inner_text() or ""):
                return True, "ok"
        except Exception:
            pass
    if detect_rate_limit(page):
        return False, "发送后检测到「操作频繁/验证码」提示，消息可能未发出"
