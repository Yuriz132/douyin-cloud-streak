...
def _js_find_and_click(page, name):
    """JS 遍历左栏 textContent，对抖音把昵称拆成单个 <span> 的情况免疫。

    抖音会把「梁登辉」拆成三个 <span>，导致 Playwright 的 get_by_text 整串匹配
    返回 0。这里直接扫描每个元素的 textContent，找包含该昵称、且自身文本最短的元素
    （即最贴近昵称的那个容器），点击它即可打开会话。"""
    try:
        handle = page.evaluate_handle("""(n) => {
            let best = null, bestLen = 1e9;
            const all = document.querySelectorAll('*');
            for (const el of all) {
                const r = el.getBoundingClientRect();
                if (r.x > 480) continue;            // 只看左侧会话列表区域
                const t = (el.textContent || '').trim();
                if (t.includes(n) && t.length > 0 && t.length < bestLen) {
    ...
    with sync_playwright() as p:
        ...
        if len(target_names) > 0:
            for name in target_names:
                print(f"🔍 寻找好友: {name}")
                # 优先用 JS 扫描（对昵称被拆成 <span> 免疫）；失败再退回到搜索框
                if _js_find_and_click(page, name):
                    print(f"👉 已打开: {name}")
                    time.sleep(random.uniform(2, 4))
                    send_current(name)
                elif search_and_open(page, name):
