...
if command -v apt-get &>/dev/null; then
    if ! timeout 10 apt-get update -o Acquire::http::Timeout=8 -o Acquire::Retries=1 >/dev/null 2>&1; then
...
EOF
        echo "[✓] 原 apt 源超时不可用，已收敛为 mirrors.aliyun.com 单源（并禁用海外附加源，避免 update 被拖挂）"
    fi
...
"$VENV/bin/pip" install -r "$SERVICE_DIR/requirements.txt" -i https://mirrors.aliyun.com/pypi/simple/
