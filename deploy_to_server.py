...
def sync_local_data(ssh, remote_dir: str) -> list[str]:
    ...
    if uploaded:
        print("[*] 数据已上传，正在重启服务让台账生效...")
        stdin, stdout, stderr = ssh.exec_command(
            "systemctl restart douyin-cloud-streak 2>/dev/null || true"
        )
        stdout.channel.recv_exit_status()
        print("[✓] 服务已重启，登录态与联系人信息已生效！")
...
def main():
    ...
    try:
        ...
        if exit_status == 0:
            ...
            print("=" * 65)
            print("【接下来只需】：")
            print(f"1. 浏览器打开: http://{server_ip}:8000，输入上面令牌；")
