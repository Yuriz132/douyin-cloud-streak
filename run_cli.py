...
def shutdown_executor_cleanly() -> None:
    """先在工作线程内回收浏览器池（关闭 Chromium 与 playwright 驱动），再停线程池。

    跳过池回收直接退出时，playwright node 驱动会因 stdio 管道断裂抛出
    EPIPE 未处理异常，在控制台刷出大段无关堆栈。
    """
    try:
...
def main():
    ...
    if args.sync_contacts:
        print("\n[*] 正在启动浏览器同步抖音联系人列表...")
        res = executor.submit_and_wait(lambda: sync_contacts(account_id))
        if res.get("error"):
    ...
    res = executor.submit_and_wait(
        lambda: run_send(dry_run=args.dry_run, only_names=only_names, account_id=account_id)
    )
